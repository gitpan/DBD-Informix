...
#      }
    ...
    for ( i = 1 ; i <= imp_sth->fbh_num ; i++ ) {
        ...
        if ( dbis->debug >=2 ) {
            printf( "In: DBD::Informix::fetchrow'FieldBufferDump: %d\n", i );
            printf( "In: DBD::Informix::fetchrow'FieldBufferDump->cbuf: %s\n", 
                    fbh->cbuf );
            printf( "In: DBD::Informix::fetchrow'FieldBufferDump->rlen: %i\n", 
                    fbh->rlen );
          }
